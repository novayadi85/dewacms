var apps = angular.module('metafield', ['datatables']);
...
apps.controller('metafields', ['$scope', '$http', '$location', '$window', function ($scope, $http , $location ,$window) {
    $scope.post = {
        'title' : '',
        'handle' : '',
		'post_type' : '',
    ...
    $scope.search = function () {
        ...
        $('body').progress('open');
        $http.post("/admin/metafields/api_v1",{params:$scope.post})
    ...
    $scope.deleteData = function(params) {
        ...
        .then((willDelete) => {
          if (willDelete) {
              $http.post("metafields/remove",{params: item, action: 'remove'})
                .then(function(response) {
                    $scope.search();
    ...
    $scope.editData = function(){
        ...
        $window.location.href = "/admin/metafields/show/" + item['id'] ;
     };
    ...
    $scope.submit = function(){
        ...
        $http.post("/admin/metafields/store",{params:$scope.post , action:'add'})
            .then(function (response) {
