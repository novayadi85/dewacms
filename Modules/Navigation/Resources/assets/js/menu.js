var apps = angular.module('navigation', ['datatables','localytics.directives']);
	...
	apps.controller('organizer', ['$scope', '$http', function ($scope, $http) {
			...
			$scope.reloadList = function(details) {
				var items  = $(details.sourceEl).html('output');
				$('#nestable_lists li').each(function(index){
					//console.log(index)
				});
				var ab = $('#nestable_lists').nestable('serialize')
				//console.log(ab)
				$('body').progress('open');
				$http.post("/admin/menu/reload",{params:$scope.menu , data: (ab), action: 'save' , details: details})
					.then(function(response) {
						$scope.search();
					})
					.finally(function () {
